...
export const fetchEmployees = () => {
  return (dispatch, getState) => {
    ...
    const role_id = state.auth.user ? state.auth.user.role_id : null;
    dispatch(fetchEmployeesRequest());

    return axios.get("http://localhost:8080/api/user", {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_id || 'default',  // Gửi Site-Type trong header
...
export const editEmployee = (id, employeeData) => {
  return (dispatch, getState) => {
    ...
    return axios.put(`http://localhost:8080/api/user/${id}`, employeeData, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_id || 'default',  // Gửi Site-Type trong header
        'Content-Type': 'application/json'
...
export const addEmployee = (employeeData) => {
  return (dispatch, getState) => {
    ...
    return axios.post("http://localhost:8080/api/user", employeeData, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_id || 'default',  // Gửi Site-Type trong header
        'Content-Type': 'application/json'
...
export const updateEmployeeStatus = (id) => {
  return (dispatch, getState) => {
    ...
    return axios.put(`http://localhost:8080/api/user/${id}/disable`, {},{
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_id || 'default',  // Gửi Site-Type trong header
        'Content-Type': 'application/json'
...
export const checkEmailExists = (email) => {
  return (dispatch, getState) => {
    ...
    return axios.post(`http://localhost:8080/api/user/CheckEmail`, email, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_id || 'default',
        'Content-Type': 'application/json'
      }
    })
      .then(response => {
