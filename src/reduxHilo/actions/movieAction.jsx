...
export const fetchMovies = () => {
  return (dispatch, getState) => {
    ...
    const role_name = state.auth.user ? state.auth.user.role_name : null;
    dispatch(fetchMoviesRequest());
    console.log(token);
    console.log(role_name);
    return axios.get("http://localhost:8080/api/movie", {
      headers: {
...
export const editMovie = (id, movieData) => {
  return (dispatch, getState) => {
    ...
    return axios.put(`http://localhost:8080/api/movie/${id}`, movieData, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_name || 'default',
...
export const fetchMovieDetails = (id) => {
  return (dispatch) => {
    return axios.get(`http://localhost:8080/api/movie/${id}`)
      .then(response => {
...
export const addMovie = (movieData) => {
  return (dispatch, getState) => {
    ...
    return axios.post("http://localhost:8080/api/movie/new", movieData, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_name || 'default',
        'Content-Type': 'application/json',
...
export const hiddenMovie = (id) => {
  return (dispatch, getState) => {
    ...
    return axios.put(`http://localhost:8080/api/movie/${id}/disable`, {}, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Site-Type': role_name || 'default',
        'Content-Type': 'application/json',
...
export const fetchMoviesCount = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get('http://localhost:8080/api/movie/count', {
        headers: {
